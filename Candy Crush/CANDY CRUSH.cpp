...
void save(int grid[][max_col], int rows, int cols) {
    ofstream file("save.txt");// for inserting ham ofstream use krty hain 
    if (!file)
        return;

    file << rows << " " << cols << endl; // 6 6
    file << score << " " << remainingmoves << endl;// 500 13
    for (int r = 0; r < rows; r++) {            
        for (int c = 0; c < cols; c++) {
            file << grid[r][c] << " ";        // 0 4 3 2 8 1 
                                             // 5 6 2 4 3 1
        }                                   //  6 4 3 3 2 6
        file << "\n";
    }
...
void runGame(int grid[][max_col], int rows, int cols, bool condition)
{
    ...
    Texture textures[8];//--------------------------images will be loaded here 
    for (int i = 0; i < 8; i++)
    {


        string filename = "candy" + to_string(i) + ".png";//candy0.png   candy1.png
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                int mx = event.mouseButton.x;//horizontal coordinate 
                int my = event.mouseButton.y;//y coordinate
                ...
                if (my > rows * cellSize)// kahin user ka click window sa bhr to nai 
                    continue;

                int i = my / cellSize;//row index 
                int j = mx / cellSize;//col ind 

                if (i < 0 || i >= rows || j < 0 || j >= cols)//if out of window 
                    continue;
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                ...
                //images will be paste on grid[0][0],grid[0][1] in this part 
                Sprite sp;
                sp.setTexture(textures[val]);
                float scaleX = (float)cellSize / textures[val].getSize().x;//textures[val]=jo candy image use ho gi 
                float scaleY = (float)cellSize / textures[val].getSize().y;//gestsize().x image ki width 
...
int main()
{
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(e))
        {
            ...
            bool condition = false;// for grid ta k us ko save kr skyn 
